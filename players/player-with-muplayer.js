...
var REQUEST_URLS = {
	...
	MUPLAYER_STATIC_RES: '/api_res/js/libs/muplayer/' 
};
...
var RadioPlayer = function(container) {
	var host = '';//Utils.getHost(__uri('/api_res/js/libs/muplayer/muplayer_mp3.swf'));
	...
	// TODO: for test
	host = 'http://webapi.br.baidu.com';
	
	this._currentVolume = 50;
	this._player = new _mu.Player({
		baseDir: host + REQUEST_URLS.MUPLAYER_STATIC_RES, 
	...
	_create: function() {
		...
		var reportBeforeUnload = function() {
			...
			if (that._playerState) {
				...
				// TODO: for test
				var img = new Image();
				img.src = 'http://www.baidu.com/cache.jpg?a=' + song['song_name'] + 
					'&b=' + (that._playerPosition ? that._playerPosition : 0) + 
					'&c=' + that.curRadio.name;
				
				radioReporter.leavePage(song['song_name'], 
						(that._playerPosition ? that._playerPosition : 0), 
						that.curRadio.name);
			}
	...
	_bindPayerEvents: function() {
		...
		this._player
			...
			.on('player:next', function(data) {
				...
				if (data.auto) {
					radioReporter.nextAuto(song['song_name'], that._playerDuration * 1000, that.curRadio.name);
				} else {
					radioReporter.nextClick(song['song_name'], 
							(that._playerPosition ? that._playerPosition : 0) * 1000, 
							that.curRadio.name);
				}
			})
			.on('player:play', function() {
				...
				radioReporter.playClick(song['song_name'], that.curRadio.name);
			})
			.on('player:pause', function() {
				...
				radioReporter.pauseClick(song['song_name'], reportDuration * 1000, that.curRadio.name);
			})
...
Radios.prototype = {
	...
	_create: function() {
		...
		if (!Radios._initialized) {
			$(parent.document).add(document).on('click', function(event) {
	...
	_change: function() {
		...
		$.each(radios, function(i, radio) {
			...
			// continue to next
			if ($active) {
				if ($active.data('type') === radio.type) {
					if ($active.data('id') === radio.id) {
						return;
					}
				} else {
					if (i === 0) {
