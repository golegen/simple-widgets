/*
 *
 * Depends: jquery, jqueryui, soundmanager, modernizr
 */
var REQUEST_URLS = {
...
var Utils = {
	...
	log: function(msg) {
		...
		var $log = $('body').find('#radio-log');
		
		if (!$log.length) {
			$log = $('<div id="radio-log" style="width: 400px; height: 300px; overflow: auto; padding: 5px; position: absolute; top: 0; left: 0; z-index: 99999; border: solid 1px #ccc; background: #fff;" />').appendTo('body');
		}
...
var RadioPlayer = function(container) {
	...
	soundManager.setup({
		url: local + REQUEST_URLS.SOUND_MANAGER_SWF,
		flashVersion: 9,
		preferFlash: true
	});
	...
	this._albumErrorImage.src = local + 'css/img/album-error.png';
	this.curRadio = {};
...
RadioPlayer.prototype = {
	...
	_create: function() {
		...
		this._$buttonPlay.on('click', function(event) {
			...
			if ($button.hasClass('qx-radio-state-play')) {
				...
				// report: play click
				// console.log(store.data[store.index]['song_name'], that.curRadio.name);
			} else {
		...
		var reportBeforeUnload = function() {
			...
			if (that._playerState) {
				...
				// report: page leave
				// console.log(store.data[store.index]['song_name'], that._player.position, that.curRadio.name);
			}
	...
	_next: function(auto) {
		...
		if (!auto) {
			...
			// report: next click
			// console.log(store.data[store.index]['song_name'], this._player.position, this.curRadio.name);
	...
	_add: function() {
		...
		this._player = soundManager.createSound({
			...
			onfinish: function() {
				...
				// report: auto next
				// console.log(store.data[store.index]['song_name'], this.duration, that.curRadio.name);
			...
			onpause: function() {
				...
				//console.log(store.data[store.index]['song_name'], reportDuration, that.curRadio.name);
			},
			
			onload: function(success) {
				if (success) {
					Utils.log('load song success!');
					Utils.log('song name: ' + (store.data[store.index]['song_name']));
					Utils.log('song no: ' + (store.index + 1) + '/' + store.data.length);
				} else {
					that._next();
					
					Utils.log('<span style="color: #f00;">load song fail!</span>');
					Utils.log('<span style="color: #f00;">song name: ' + (store.data[store.index]['song_name']) + '</span>');
					Utils.log('<span style="color: #f00;">song no: ' + (store.index + 1) + '/' + store.data.length + '</span>');
				}
			}
		});
	...
	_fetch: function() {
		...
		return $.ajax(requestUrl, {
			...
			// log
			Utils.log('-----------------------------------------');
			Utils.log('fetch source success!');
			if (curRadio.type !== 'random') {
				Utils.log('source group: ' + that._playStore[storeKey].groupIndex);
			}
			Utils.log('play model: ' + (curRadio.type === 'random' ? '随机' : '列表'));
			Utils.log('song count: ' + res.data.length);
			
		}).fail(function() {
			that._$widget.trigger('error');
			
			// log
			Utils.log('<span style="color: #f00;">fetch source fail!</span>');
		});
	},
...
Radios.prototype = {
	...
	_create: function() {
		...
		this._$wrapper
			.on('click', '.qx-radio-button-expand', function(event) {
				...
				// report: expand channel pane
				// console.log('expand channels');
	...
	_select: function(index) {
		...
		if (typeof localStorage === 'object') {
			localStorage.radio = JSON.stringify({
				selectIndex: index,
				text: channel,
				data: $active.data()
			});
	...
	update: function() {
		...
		if (typeof localStorage === 'object') {
			radio = localStorage.radio ? JSON.parse(localStorage.radio) : {};
			if (typeof radio.selectIndex === 'number') {
				that.selectIndex = radio.selectIndex;
				that.visitedChannel = radio;
			}
			that._update();
		} else {
			that._update();
